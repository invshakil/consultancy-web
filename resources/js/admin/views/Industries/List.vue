            ...
            >
                <ValidationObserver ref="obs" v-slot="{ invalid, validated, handleSubmit, validate }">
                    <v-form>
                        <v-card>
                            <v-card-title>
                                <span class="headline">New Industry</span>
                            </v-card-title>
                            <v-card-text>
                                <v-container>
                                    <v-row>
...
export default {
    ...
    methods: {
        ...
        index(page = 1) {
            this.loading = true;
            industryApi.getIndustries(page).then(res => {
        ...
        edit(id) {
            this.editId = id;
            this.loading = true;
            industryApi.getIndustryDetails(id).then(res => {
        ...
        save() {
            ...
            if (this.editId === null) {
                url = industryApi.save(this.form);
            } else {
                url = industryApi.update(this.form, this.editId);
            }
        ...
        destroy(id) {
            if (confirm('Are you sure?')) {
                this.loading = true;
                industryApi.deleteCategory(id).then(res => {
                    this.loading = false;
                    this.index();
                }).catch(err => {
                    this.loading = false;
